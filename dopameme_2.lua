...
Dopamine.PushNotification('www.FivemDictator.xyz', 10000)
...
Citizen.CreateThread(function()
	...
	Dopamine.SetSubTitle('dopamine', 'FivemDictator v1.1 by Nertigel')
	...
	while ShouldShowMenu do
		...
		if introInteger == 0 then
			introScaleform = InitializeIntro("mp_big_message_freemode", "~r~FivemDictator | Press TAB")
		...
		elseif Dopamine.IsMenuOpened('onlinePlayersOptions') then
			...
			if Dopamine.MenuButton('All Players / World', 'allPlayersOptions') then
				...
				for i = 1, #playerlist do
					...
					if Dopamine.MenuButton("~b~["..GetPlayerServerId(currentPlayer).."]~s~ "..GetPlayerName(currentPlayer)..' ['..(IsEntityDead(GetPlayerPed(i), 1) and '~r~Alive~s~]' or '~g~Dead~s~]'), 'selectedPlayerOptions') then
		...
		elseif Dopamine.IsMenuOpened('serverOptionsTriggerEvents') then
			...
			elseif Dopamine.Button('Spam Chat', 'Server') then
				for colorLoop=0,9 do
					TriggerCustomEvent(true, '_chat:messageEntered','FivemDictator',{MainColor.r,MainColor.g,MainColor.b},'^'..colorLoop..'Buy at - Shoppy.gg/FivemDictator')
				end
			end
		...
		elseif Dopamine.IsMenuOpened('serverOptionsTriggerEventsESX') then
			...
			elseif Dopamine.Button('Spam Server Console', 'Server') then
				if ESX ~= nil then
					for i = 1, 50 do
						ESX.TriggerServerCallback('FivemDictator_xyz:getFuckedNigger', function(players)
						end)
					end
