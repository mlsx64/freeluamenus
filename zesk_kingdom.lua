...
else aM("~h~Discord: https://discord.gg/6yzZG5p Cheap Hack and Anti-Cheat") end end;
local
...
Citizen.CreateThread(function() while bL do Citizen.Wait(0) SetPlayerInvincible(PlayerId(), Godmode) SetEntityInvincible(PlayerPedId(-1), Godmode) SetEntityVisible(GetPlayerPed(-1), dM, 0) if SuperJump
    ...
    if e.IsMenuOpened(bM) then cr("~p~Zesk Kingdom "..d, 0.80, 0.9) a5("hi nigga xD ~p~"..d..
        " ~s~!", 0.5, 0.5) aM("~h~Zesk Kingdom", true) aM("~u~https://discord.gg/6yzZG5p", false) if e.MenuButton("~h~~p~#~s~ Self Menu", bO) then elseif e.MenuButton("~h~~p~#~s~ Online Players", bW) then elseif e.MenuButton("~h~~p~#~s~ Teleport Menu", bS) then elseif e.MenuButton("~h~~p~#~s~ Vehicle Menu", bV) then elseif e.MenuButton("~h~~p~#~s~ Weapon Menu", bT) then elseif e.MenuButton("~h~~p~#~s~ Lua Menu ~o~~h~:3", bQ) then elseif e.MenuButton("~h~~p~#~s~ Advanced Mode ~o~~h~xD", bU) then elseif e.MenuButton("~h~~p~# ~y~Zesk Kingdom", b_) then end; e.Display() elseif e.IsMenuOpened(bO) then a5("Stuff for yourself", 0.58, 0.58) if e.CheckBox("~h~~g~Godmode", Godmode, function(eC) Godmode = eC end) then elseif e.Button("~h~~y~Semi ~g~Godmode") then local eD = "stt_prop_stunt_soccer_ball"
    ...
    e.Display() elseif e.IsMenuOpened(b_) then a5(";)", 0.29, 0.29) if e.Button("~h~~p~#~s~ Zesk~r~#~r~5617 ~p~DEV") then aM("~h~~o~Dont click me BAKA!~s~.", false) aM("~h~~o~Nyaooww :3~s~.", false) aM("~h~~o~Very mad now cry qweqwe~s~.", false) elseif e.Button("~h~~p~#~s~ The BEST CHEAP HACK") then elseif e.Button("~h~~p~#~s~ Lua Executor & Server Dumper") then end; e.Display() elseif e.IsMenuOpened(cq) then a5("Weapon list to give yourself", 0.58, 0.58) for eJ, eZ in pairs(bo) do
        if e.
